...
public class Puzzle {
	...
	public static void run(File scriptFile, Displacement displacement) {
		try {
			...
			for(int i = 0; i < images.length; i++) {
				InputStream in = new FileInputStream(new File(scriptFile.getParent(), images[i].get("image")));
				textures[i] = TextureLoader.getTexture("PNG", in);
				regions[i] = Region.createRegion(textures[i]);
				points[i] = new Point(
					Integer.parseInt(images[i].get("x")),
					Integer.parseInt(images[i].get("y"))
				);
			}
	...
	private static void renderImage(Texture texture, int x, int y) {
		...
		//GL11.glTexParameterf( GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR_MIPMAP_NEAREST );
	    // when texture area is large, bilinear filter the first mipmap
		// GL11.glTexParameterf( GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR );

	    
		// GL11.glTexParameterf( GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, GL11.GL_CLAMP );
